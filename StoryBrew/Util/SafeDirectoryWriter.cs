...
namespace StoryBrew.Util
{
    public class SafeDirectoryWriter : IDisposable
    {
        ...
        public void Commit(bool checkPaths = true)
        {
            ...
            {
                if (paths.Count == 0)
                    throw new InvalidOperationException($"No file path requested");
                ...
                foreach (var path in paths)
                {
                    ...
                    if (!file.Exists)
                        throw new InvalidOperationException($"File path requested but not created: {path}");

                    if (file.Length == 0)
                        throw new InvalidOperationException($"File path requested but is empty: {path}");
                }
            }
